...
def clean_letter_response(text: str) -> str:
    """
    Очищает ответ от лишних пояснений, но не обрезает русский текст.
    Если после очистки остаётся пустота – возвращает исходный текст (обрезанный до 1200 символов).
    """
    if not text:
        return ""
    ...
    if not has_cyrillic:
        # Если нет русских букв, просто возвращаем обрезанный чистый текст
        return re.sub(r'\s+', ' ', text).strip()[:1200]

    # Разбиваем на строки и удаляем явные "размышления"
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
        stripped = line.strip()
        ...
        if re.match(r'^(We need|The user|Let\'s|I need|Here is|I will|We\'ll|Okay,|So,|First,|Now,|Sure,|Alright,)', stripped):
            continue
        cleaned_lines.append(stripped)

    if not cleaned_lines:
        # Если все строки были удалены, возвращаем исходный текст, обрезав лишние пробелы
        return re.sub(r'\s+', ' ', text).strip()[:1200]
...
def generate_cover_letter(
    ...
    # Если после генерации письмо пустое – используем fallback
    if not letter.strip():
        print("Warning: generated letter is empty, using fallback.")
...
def generate_openrouter_letter(llm: LlmConfig, system: str, user: str, vacancy: Vacancy) -> str:
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=600,  # увеличено с 450 для более полного ответа
            extra_headers={
                "HTTP-Referer": "https://github.com/sergik15828/hh-auto-apply",
                "X-Title": "hh-auto-apply",
            },
        ...
        if cleaned:
            ...
            # Если очистка дала пустоту, используем сырой текст (обрезаем) или fallback
            return raw_text[:1200] if raw_text else generate_fallback_letter(vacancy)
    except Exception as e:
...
def apply_to_vacancy(page: Page, vacancy: Vacancy, letter: str, config: dict[str, Any]) -> tuple[str, str]:
    # Если письмо пустое, пытаемся сгенерировать fallback прямо здесь
    if not letter.strip():
        print("Letter is empty, generating fallback in apply_to_vacancy...")
        letter = generate_fallback_letter(vacancy)
        if not letter.strip():
            return "error", "No letter generated"
...
def run_once(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    try:
        ...
        for vacancy in vacancies:
            ...
            # Генерация письма с проверкой на пустоту
            letter = generate_cover_letter(llm, profile, vacancy, config)
            if not letter.strip():
                # Если письмо всё ещё пустое, пропускаем эту вакансию с ошибкой
                error_msg = "Letter generation failed (empty result)"
