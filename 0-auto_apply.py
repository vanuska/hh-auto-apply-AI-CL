...
# <--- НОВОЕ: функция отправки уведомлений в Telegram (с поддержкой HTML)
...
def interactive_model_selection(llm: LlmConfig, profile: str) -> str:
    ...
    """
    # Актуальные модели на июль 2026
    models_to_try = [
        "openrouter/free",
        "gpt-oss-20b:free",
...
def generate_fallback_letter(vacancy: Vacancy) -> str:
    ...
    if any(word in title for word in ("devops", "infra", "инфра", "администр")):
        focus.append("управление IT-инфраструктурой, внедрение 1С и CRM, бюджетирование")
    if any(word in title for word in ("security", "безопасн", "информацион", "защит")):
        focus.append("информационная безопасность, внедрение ISO 27001 и управление рисками")
    if any(word in title for word in ("manager", "менеджер", "директор")):
        focus.append("управление IT-отделом, бюджетирование и стратегическое планирование")

    if not focus:
        focus_text = "управление IT-инфраструктурой, бюджетирование и внедрение стандартов качества"
    else:
        focus_text = "; ".join(focus[:2])

    return (
        f"Здравствуйте. По вакансии {vacancy.title} вижу хорошее совпадение с моим опытом: {focus_text}. "
        "У меня 18+ лет в управлении ИТ и Информационной Безопасности. "
        "Руководил распределёнными командами от 10 до 45+ сотрудников, внедрял 1С, CRM и ERP системы, "
        "проводил бюджетирование и тендеры. Внедрил стандарты ISO 27001 и ISO 9001. "
        "Сопоровождал ЦОД и удаленные офисы с гетерогенной ИТ, ИБ инфраструктурой. "
        "Готов обсудить задачи команды и принести пользу вашему бизнесу."
    )
...
def generate_openrouter_letter(llm: LlmConfig, system: str, user: str, vacancy: Vacancy) -> str:
    """Generate letter using OpenRouter API."""
    from openai import OpenAI
    ...
    raw_text = (response.choices[0].message.content or "").strip()
    return clean_letter_response(raw_text)
...
def run_once(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    # <--- НОВОЕ: список для хранения деталей успешных откликов
    applied_details = []
    ...
    try:
        ...
        for vacancy in vacancies:
            ...
            if args.apply:
                ...
                # <--- НОВОЕ: если успешно, сохраняем детали
                if status == "success":
    ...
    # <--- НОВОЕ: формируем сообщение со списком откликов
    ...
    if vacancies:
        ...
        # Если есть успешные отклики, добавляем список
        if applied_details:
            msg_lines.append("")
            msg_lines.append("<b>📋 Список отправленных откликов:</b>")
            # Показываем не более 10, чтобы не превысить лимит сообщения
            max_display = 10
