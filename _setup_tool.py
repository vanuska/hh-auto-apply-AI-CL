...
class SetupTool:
    ...
    def _check_gui(self):
        ...
        if not self.is_linux:
            return True  # Windows/Mac обычно имеют GUI
        # Проверяем переменную DISPLAY
        if os.environ.get("DISPLAY"):
            return True
        # Проверяем наличие X server через xdpyinfo
        try:
    ...
    def show_menu(self):
        ...
        print()
        print("  1. Установка всех зависимостей (включая xvfb для Linux-сервера)")
        ...
        print("  10. Запуск auto_apply.py (автоматически использует xvfb-run при необходимости)")
        print("  11. Выход")
        print()
        print("-" * 70)
    ...
    def _step_install_deps(self):
        ...
        # Установка xvfb для Linux-сервера без GUI
        if self.is_linux and not self.has_gui:
    ...
    def _ensure_xvfb(self):
        ...
        try:
            ...
            print("Или использовать headless-режим (добавьте --headless при запуске)")
...
"""
    ...
    def _step_run_apply(self):
        ...
        # Если система Linux без GUI, предлагаем выбрать headless или использовать xvfb
        if self.is_linux and not self.has_gui:
            print("\nОбнаружена система Linux без графического интерфейса.")
        ...
        # Если используется xvfb, проверяем его наличие
        if use_xvfb:
            try:
                subprocess.run(["which", "xvfb-run"], capture_output=True, check=True)
            except:
        ...
        # Запускаем скрипт с учетом xvfb
