...
"""
    ...
    def _step_run_apply(self):
        ...
        # Читаем текущий лимит из config.yaml
        config_file = MY_DIR / "config.yaml"
        current_limit = 5
        if config_file.exists():
            try:
        ...
        print("(можно изменить в config.yaml -> limits.max_applications_per_run)")
        print()
        
        mode = input("Выберите режим (1-5): ").strip()
        
        # Запрашиваем количество откликов
            ...
            try:
                ...
                if config_file.exists():
                    ...
                    if "limits" not in cfg:
                        cfg["limits"] = {}
                    cfg["limits"]["max_applications_per_run"] = max_apps
                    config_file.write_text(yaml.dump(cfg, allow_unicode=True), encoding="utf-8")
                    print(f"✅ Лимит обновлён: {max_apps} откликов за запуск")
            except ValueError:
                max_apps = current_limit
        ...
        if mode == "1":
            ...
            # Prod-run с видимым браузером
            args = ["--once", "--apply"]
            print("\n🚀 Запуск PROD-RUN (отклики, браузер видимый)")
            
        elif mode == "3":
            # Prod-run с headless браузером
            args = ["--once", "--apply", "--headless"]
            print("\n🚀 Запуск PROD-RUN (отклики, браузер в фоне)")
            
        elif mode == "4":
            # Расписание с видимым браузером
            args = ["--schedule", "--apply"]
            print("\n🔄 Запуск ПО РАСПИСАНИЮ (отклики, браузер видимый)")
            print(f"Время запусков: из config.yaml -> schedule.run_times")
            
        elif mode == "5":
            # Расписание с headless браузером
        ...
        # Проверка на серверный Linux без GUI
        if self.is_linux and not self.has_gui:
            # Если выбран режим с видимым браузером (2 или 4) и нет GUI
            if mode in ["2", "4"]:
                print("\n⚠️ Обнаружена система Linux без графического интерфейса.")
                print("Для видимого режима требуется xvfb-run.")
                xvfb_choice = input("Использовать xvfb-run? (Y/N, Enter=Y): ").strip().upper()
                if xvfb_choice != "N":
                    use_xvfb = True
                    # Убираем --headless если он есть
                    if "--headless" in args:
                        args.remove("--headless")
                    print("✅ Будет использован xvfb-run")
                else:
                    # Если не используем xvfb, добавляем headless
    ...
    def _run_script(self, script_name: str, args: List[str] = None, use_xvfb: bool = False):
        ...
        cmd = []
        if use_xvfb:
            # Проверяем наличие xvfb-run
            try:
                subprocess.run(["which", "xvfb-run"], capture_output=True, check=True)
                cmd = ["xvfb-run", "-a", sys.executable, str(script_path)]
            except:
                print("⚠️ xvfb-run не найден. Запуск без xvfb.")
