...
# <--- НОВОЕ: функция отправки уведомлений в Telegram
...
def run_once(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    if vacancies:
        ...
        applied = sent_or_planned if args.apply else 0
        dry_run = sent_or_planned if not args.apply else 0
        msg = (
            f"✅ Поиск завершён.\n"
            f"Найдено вакансий: {total_found}\n"
            f"Отправлено откликов: {applied}\n"
            f"Симуляций (dry-run): {dry_run}\n"
            f"Лимит: {max_per_run}\n"
            f"Время: {dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        send_telegram_notification(msg)
    else:
        send_telegram_notification("⚠️ Поиск не нашёл новых вакансий.")
    # --->
